...
def Count_bases_and_percentages(Fasta_q, Nucleo_Amino_dict = {"A":0,"C":0,"G":0,"T":0,"N":0}):
    """
    ...
    Residues, Nucleo_Amino_s = 0, Nucleo_Amino_dict
    for i in range(len(Fasta_q)):
        Residues += float(len(Fasta_q[i][1]))
        for Nucleo_Amino in Nucleo_Amino_s: Nucleo_Amino_s[Nucleo_Amino] += float(Fasta_q[i][1].count(Nucleo_Amino))

    return "{:,}".format(int(Residues)).replace(",","."),\
           tuple([int((Nucleo_Amino_s[Nucleo_Amino] / Residues) * 100) for Nucleo_Amino in Nucleo_Amino_s])
